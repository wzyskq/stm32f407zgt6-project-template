...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
    ...
    for (i = 0; i < Count; i++)     // 循环Count次，进行连续的数据写入
...
void oled_init(void)
{
    ...
    OLED_WriteCommand(0x3F); // 0x0E..0x1F  /* 0x3F对应64行 / 0x1F对应32行 */ // ************************************ ************************************ 高度像素
    ...
    OLED_WriteCommand(0x12); /* 0x12为64行模式 / 0x02为32行模式 */ // ************************************ ************************************ 高度像素
 ...
 * \note       OLED默认的Y轴，只能8个Bit为一组写入，即1页等于8个Y轴坐标
 */
 ...
 * \param[in]  X 指定点的X坐标
 * \param[in]  Y 指定点的Y坐标
 * \param[in]  StartAngle 起始角度，范围：-180..180
 * \param[in]  EndAngle 终止角度，范围：-180..180
 ...
 * \note   - 所有的显示函数，都只是对OLED显存数组进行读写
 *         - 调用OLED_Update函数或OLED_UpdateArea函数才会将显存数组的数据发送到OLED硬件，进行显示
 */
 ...
 * \param[in]  X 指定区域左上角的横坐标，范围：-32768..32767，屏幕区域：0..127
 * \param[in]  Y 指定区域左上角的纵坐标，范围：-32768..32767，屏幕区域：0..63
 * \param[in]  Width 指定区域的宽度，范围：0..128
 * \param[in]  Height 指定区域的高度，范围：0..64
 * \note       - 此函数会至少更新参数指定的区域
 *             - 如果更新区域Y轴只包含部分页，则同一页的剩余部分
 */
...
/******************************************************************
 * \brief  将OLED显存数组全部清零
 * \note   调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 */
void oled_clear(void)
{
    uint8_t i, j;
    for (j = 0; j < 8; j++)               // 遍历8页 // ************************************ ************************************ 高度更改
...
void oled_clear_area(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
    ...
    for (j = Y; j < Y + Height; j++)                             // 遍历指定页
        for (i = X; i < X + Width; i++)                          // 遍历指定列
            if (i >= 0 && i <= 127 && j >= 0 && j <= 63)         // 超出屏幕的内容不显示 // ************************************ ************************************ 高度像素
...
/**
 * \brief      OLED显示一个字符
 * \param[in]  X 指定字符左上角的横坐标，范围：-32768..32767，屏幕区域：0..127
 * \param[in]  Y 指定字符左上角的纵坐标，范围：-32768..32767，屏幕区域：0..63
...
void oled_show_char(int16_t X, int16_t Y, char Char, uint8_t FontSize)
{
    if (FontSize == OLED_8X16)                                // 字体为宽8像素，高16像素
        oled_show_image(X, Y, 8, 16, OLED_F8x16[Char - ' ']); // 将ASCII字模库OLED_F8x16的指定数据以8*16的图像格式显示
    else if (FontSize == OLED_6X8)                            // 字体为宽6像素，高8像素
        oled_show_image(X, Y, 6, 8, OLED_F6x8[Char - ' ']);   // 将ASCII字模库OLED_F6x8的指定数据以6*8的图像格式显示
}

/******************************************************************
 * \brief      OLED显示字符串（支持ASCII码和中文混合写入）
 * \param[in]  X 指定字符串左上角的横坐标，范围：-32768..32767，屏幕区域：0..127
 * \param[in]  Y 指定字符串左上角的纵坐标，范围：-32768..32767，屏幕区域：0..63
 * \param[in]  String 指定要显示的字符串，范围：ASCII码可见字符或中文字符组成的字符串
 * \param[in]  FontSize 指定字体大小
 *             范围：OLED_8X16    宽8像素，高16像素
 *                  OLED_6X8     宽6像素，高8像素
 * \note       - 显示的中文字符需要在OLED_Data.c里的OLED_CF16x16数组定义
...
void oled_show_string(int16_t X, int16_t Y, char *String, uint8_t FontSize)
{
    ...
    while (String[i] != '\0') // 遍历字符串
    {
        ...
        if (CharLength == 1) // 如果是单字节字符
        {
            ...
            oled_show_char(X + XOffset, Y, SingleChar[0], FontSize);
            XOffset += FontSize;
        } else // 否则，即多字节字符
        {
            ...
            if (FontSize == OLED_8X16) // 给定字体为8*16点阵
            {
                ...
                oled_show_image(X + XOffset, Y, 16, 16, OLED_CF16x16[pIndex].Data);
                XOffset += 16;
            } else if (FontSize == OLED_6X8) // 给定字体为6*8点阵
            {
                /* 空间不足，此位置显示'?' */
                oled_show_char(X + XOffset, Y, '?', OLED_6X8);
                XOffset += OLED_6X8;
            }
        }
    }
 ...
 * \param[in]  X 指定数字左上角的横坐标，范围：-32768..32767，屏幕区域：0..127
 * \param[in]  Y 指定数字左上角的纵坐标，范围：-32768..32767，屏幕区域：0..31
 * \param[in]  Number 指定要显示的数字，范围：0..4294967295
 * \param[in]  Length 指定数字的长度，范围：0..10
 * \param[in]  FontSize 指定字体大小
 *             范围：OLED_8X16    宽8像素，高16像素
 *                  OLED_6X8     宽6像素，高8像素
 * \brief      调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 */
 ...
 * \param[in]  X 指定数字左上角的横坐标，范围：-32768..32767，屏幕区域：0..127
 * \param[in]  Y 指定数字左上角的纵坐标，范围：-32768..32767，屏幕区域：0..63
 ...
 * \param[in]  Length 指定数字的长度，范围：0..8
 * \param[in]  FontSize 指定字体大小
 *             范围：OLED_8X16    宽8像素，高16像素
 *                  OLED_6X8	 宽6像素，高8像素
 * \note       调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 ...
 * \param[in]  Y 指定数字左上角的纵坐标，范围：-32768..32767，屏幕区域：0..63
 * \param[in]  Number 指定要显示的数字，范围：0x00000000..0xFFFFFFFF
 * \param[in]  Length 指定数字的长度，范围：0..16
 * \param[in]  FontSize 指定字体大小
 *             范围：OLED_8X16    宽8像素，高16像素
 *                  OLED_6X8     宽6像素，高8像素
 * \note       调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 ...
 * \param[in]  X 指定数字左上角的横坐标，范围：-32768..32767，屏幕区域：0..127
 * \param[in]  Y 指定数字左上角的纵坐标，范围：-32768..32767，屏幕区域：0..63
 * \param[in]  Number 指定要显示的数字，范围：-4294967295.0..4294967295.0
 * \param[in]  IntLength 指定数字的整数位长度，范围：0..10
 * \param[in]  FraLength 指定数字的小数位长度，范围：0..9，小数进行四舍五入显示
 * \param[in]  FontSize 指定字体大小
 *             范围：OLED_8X16    宽8像素，高16像素
 *                  OLED_6X8     宽6像素，高8像素
 * \note       调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
...
/******************************************************************
 * \brief      OLED显示图像
 * \param[in]  X 指定图像左上角的横坐标，范围：-32768..32767，屏幕区域：0..127
 * \param[in]  Y 指定图像左上角的纵坐标，范围：-32768..32767，屏幕区域：0..63
 * \param[in]  Width 指定图像的宽度，范围：0..128
 * \param[in]  Height 指定图像的高度，范围：0..64
 * \param[in]  Image 指定要显示的图像
 * \note       调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 */
 ...
 * \param[in]  X 指定格式化字符串左上角的横坐标，范围：-32768..32767，屏幕区域：0..127
 * \param[in]  Y 指定格式化字符串左上角的纵坐标，范围：-32768..32767，屏幕区域：0..63
 * \param[in]  FontSize 指定字体大小
 *             范围：OLED_8X16    宽8像素，高16像素
 *                   OLED_6X8    宽6像素，高8像素
 * \param[in]  format 指定要显示的格式化字符串，范围：ASCII码可见字符或中文字符组成的字符串
 * \param[in]  ... 格式化字符串参数列表
 * \note       - 显示的中文字符需要在OLED_Data.c里的OLED_CF16x16数组定义
 *             - 未找到指定中文字符时，会显示默认图形（一个方框，内部一个问号）
 *             - 当字体大小为OLED_8X16时，中文字符以16*16点阵正常显示
...
/******************************************************************
 * \brief      OLED在指定位置画一个点
 * \param[in]  X 指定点的横坐标，范围：-32768..32767，屏幕区域：0..127
 * \param[in]  Y 指定点的纵坐标，范围：-32768..32767，屏幕区域：0..63
 * \note       调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 */
...
/******************************************************************
 * \brief      OLED画线
 * \param[in]  X0 指定一个端点的横坐标，范围：-32768..32767，屏幕区域：0..127
 * \param[in]  Y0 指定一个端点的纵坐标，范围：-32768..32767，屏幕区域：0..63
 * \param[in]  X1 指定另一个端点的横坐标，范围：-32768..32767，屏幕区域：0..127
 * \param[in]  Y1 指定另一个端点的纵坐标，范围：-32768..32767，屏幕区域：0..63
 * \note       调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 */
...
/******************************************************************
 * \brief      OLED矩形
 * \param[in]  X 指定矩形左上角的横坐标，范围：-32768..32767，屏幕区域：0..127
 * \param[in]  Y 指定矩形左上角的纵坐标，范围：-32768..32767，屏幕区域：0..63
    ...
    } else // 指定矩形填充
    {
        for (i = X; i < X + Width; i++)      // 遍历X坐标
            for (j = Y; j < Y + Height; j++) // 遍历Y坐标
                oled_draw_point(i, j);       // 在指定区域画点，填充满矩形
    }
}

/******************************************************************
 * \brief      OLED三角形
 * \param[in]  X0 指定第一个端点的横坐标，范围：-32768..32767，屏幕区域：0..127
 * \param[in]  Y0 指定第一个端点的纵坐标，范围：-32768..32767，屏幕区域：0..63
 * \param[in]  X1 指定第二个端点的横坐标，范围：-32768..32767，屏幕区域：0..127
 * \param[in]  Y1 指定第二个端点的纵坐标，范围：-32768..32767，屏幕区域：0..63
 * \param[in]  X2 指定第三个端点的横坐标，范围：-32768..32767，屏幕区域：0..127
 * \param[in]  Y2 指定第三个端点的纵坐标，范围：-32768..32767，屏幕区域：0..63
...
/******************************************************************
 * \brief      OLED画圆
 * \param[in]  X 指定圆的圆心横坐标，范围：-32768..32767，屏幕区域：0..127
 * \param[in]  Y 指定圆的圆心纵坐标，范围：-32768..32767，屏幕区域：0..63
...
void oled_draw_circle(int16_t X, int16_t Y, uint8_t Radius, uint8_t IsFilled)
{
    ...
    if (IsFilled)                      // 指定圆填充
        for (j = -y; j < y; j++)       // 遍历起始点Y坐标
            oled_draw_point(X, Y + j); // 在指定区域画点，填充部分圆
...
/******************************************************************
 * \brief      OLED画椭圆
 * \param[in]  X 指定椭圆的圆心横坐标，范围：-32768..32767，屏幕区域：0..127
 * \param[in]  Y 指定椭圆的圆心纵坐标，范围：-32768..32767，屏幕区域：0..63
...
/******************************************************************
 * \brief      OLED画圆弧
 * \param[in]  X 指定圆弧的圆心横坐标，范围：-32768..32767，屏幕区域：0..127
 * \param[in]  Y 指定圆弧的圆心纵坐标，范围：-32768..32767，屏幕区域：0..63
 * \param[in]  Radius 指定圆弧的半径，范围：0..255
 * \param[in]  StartAngle 指定圆弧的起始角度，范围：-180..180
 *             水平向右为0度，水平向左为180度或-180度，下方为正数，上方为负数，顺时针旋转
 * \param[in]  EndAngle 指定圆弧的终止角度，范围：-180..180
 *             水平向右为0度，水平向左为180度或-180度，下方为正数，上方为负数，顺时针旋转
