...
u8 sdcReFlag            = 1;   // 调试模式：1 开启，0 关闭
u8 sdcBuf[SDC_RBUF_LEN] = {0}; // SD卡只读缓存
FRESULT mounted         = 0;   // 挂载结果
...
FRESULT sdc_get(u32 *allCapacity, u32 *freeCapacity, sdcStgUnit_t unit)
{
    ...
    FRESULT freeSpace = f_getfree("", &freeClust, &pfs);
    ...
    if (freeSpace == FR_OK) {
        ...
        if (freeCapacity)
            *freeCapacity = 0;
        if (sdcReFlag)
            serial_printf(1, "Failed to get SD card free space. Error: (%d)\r\n", freeSpace);
    }
    return freeSpace;
}
...
void sdc_open(u8 *path)
{
    ...
    } else // 路径相同
    {
        if (f_isopen(&fdst)) {
            if (sdcReFlag)
                serial_printf(0, "File (%s) is already open.\r\n", path);
            return; // 文件已打开，直接返回
        }
    }
    ...
    while (attempts < 5) {
        ...
        if (fr == FR_OK) {
            ...
            // strcpy((u8 *)prePath, path);       // 更新历史路径

            snprintf(prePath, SDC_PBUF_LEN - 1, "%s", path); // 更新历史路径

            if (sdcReFlag)
                serial_printf(0, "File (%s) opened successfully.\r\n", path);
    ...
    if (sdcReFlag)
        serial_printf(0, "Failed to open file (%s). Error: (%d)\r\n", path, fr);
    return; // 打开失败，返回
}
...
void sdc_close(void)
{
    if (f_isopen(&fdst)) {
        ...
        if (sdcReFlag)
            serial_printf(0, "File (%s) closed successfully.\r\n", prePath);
    }
}
...
void sdc_delete(u8 *path)
{
    ...
    if (res == FR_OK) {
        if (sdcReFlag)
            serial_printf(0, "File (%s) deleted successfully.\r\n", path);
        return; // 删除成功，返回
    }

    if (sdcReFlag)
        serial_printf(0, "Failed to delete (%s). Error: (%d)\r\n", path, res);
    return; // 删除失败，返回
}
...
void sdc_mkdir(u8 *path)
{
    ...
    if (f_stat(path, NULL) == FR_OK) {
        if (sdcReFlag)
            serial_printf(0, "Directory (%s) already exists.\r\n", path);
        return;
    }
    // 尝试创建目录，最多5次
    while (attempts < 5) {
        ...
        if (res == FR_OK) {
            if (sdcReFlag)
                serial_printf(0, "Directory (%s) created successfully.\r\n", path);
            return; // 创建成功，返回
        }
        attempts++;
    }
    ...
    if (sdcReFlag)
        serial_printf(0, "Failed to create directory (%s). Error: (%d)\r\n", path, res);
    return; // 创建失败，返回
}
 ...
 *   \arg        READ_LINE: 读取一行
 *   \arg        READ_FILE: 读取全部
 * \param[in]  lineNum: 读取的行号（仅在 READ_LINE 模式下有效）
 *   \arg        =0: 读取当前光标所在行
 *   \arg        >0: 读取指定行号
 * \retval     1: 成功，0: 失败
 *
 ...
 */
u8 sdc_read(sdcReadMode_t mode, u32 lineNum)
{
    if (!f_isopen(&fdst)) {
        if (sdcReFlag)
            serial_printf(0, "File (%s) is not open. Please open it first.\r\n", prePath);
        return 0;
    }

    if (mode == READ_LINE) // 读取一行
    {
        u8 *frLine = NULL; // f_gets 返回值（即输入的缓冲区指针）
        ...
        if (lineNum == 0) {
            ...
            if (frLine == NULL) {
                if (sdcReFlag)
                    serial_printf(0, "Failed to read current line from file (%s).\r\n", prePath);
                return 0; // 读取失败或到达文件末尾
            }
            curLine++;

            if (sdcReFlag)
                serial_printf(0, "Read current line %u (%d B): \r\n%s\r\n", curLine, strlen(sdcBuf), sdcBuf);
            return 1; // 读取成功
        } else {
            ...
            while (curLine < lineNum) {
                ...
                if (frLine == NULL) {
                    if (sdcReFlag)
                        serial_printf(0, "Failed to read line %u from file (%s).\r\n", lineNum, prePath);
                    return 0; // 读取失败或到达文件末尾
                }
                curLine++;
            }
            ...
            if (sdcReFlag)
                serial_printf(0, "Read line %u (%d B): \r\n%s\r\n", curLine, strlen(sdcBuf), sdcBuf);
            return 1; // 读取成功
        }
    } else if (mode == READ_FILE) // 读取全部
    {
        f_lseek(&fdst, 0);
        fr = f_read(&fdst, sdcBuf, sizeof(sdcBuf) - 1, &br);
        if (fr == FR_OK && br > 0) {
            sdcBuf[br] = '\0'; // 手动添加字符串结束符，防止附带之前的数据
            if (sdcReFlag)
                serial_printf(0, "Read all data (%d B): \r\n%s\r\n", strlen(sdcBuf), sdcBuf);
            return 1; // 读取成功
        } else {
            if (sdcReFlag)
                serial_printf(0, "Failed to read all data from file (%s). Error: (%d)\r\n", prePath, fr);
            return 0; // 读取失败
        }
    }
}
...
void sdc_write(u8 *format, ...)
{
    ...
    if (fr == FR_OK && bw > 0) {
        if (!f_sync(&fdst)) {
            if (sdcReFlag)
                serial_printf(0, "Data written successfully to file (%s).\r\n", prePath);
            return; // 写入成功，返回
        }

        if (sdcReFlag)
            serial_printf(0, "Failed to sync file (%s) after write. Error: (%d)\r\n", prePath, fr);
        return; // 同步失败，返回
    }

    if (sdcReFlag)
        serial_printf(0, "Failed to write data to file (%s). Error: (%d)\r\n", prePath, fr);
    return; // 写入失败，返回
}
...
//
//         // 判断是否满10行
//         if (!sdc_read(READ_LINE, 10))
//         {
//             // 找到未满10行的文件，直接返回
//             found = 1;
//
//             if (sdcReFlag)
//                 serial_printf(0, "Found file (%s), open for write.\r\n", filepath);
//             f_closedir(&dj);
//             return;
//         }
//         // 关闭当前文件，继续找下一个
...
//
//         if (sdcReFlag)
//             serial_printf(0, "All files full or no file, created and opened new file: %s\r\n", newfile);
//     }
// }
//
// /**
...
//     {
//         if (sdcReFlag)
//             serial_printf(0, "Failed to open log file (%s).\r\n", filepath);
//         return; // 打开失败，返回
//     }
//
...
//         {
//             if (sdcReFlag)
//                 serial_printf(0, "Log data written successfully to file (%s).\r\n", filepath);
//             return; // 写入成功，返回
//         }
//
//         if (sdcReFlag)
//             serial_printf(0, "Failed to sync log file (%s) after write.\r\n", filepath);
//         return; // 同步失败，返回
//     }
//
//     if (sdcReFlag)
//         serial_printf(0, "Failed to write log data to file (%s).\r\n", filepath);
//     return; // 写入失败，返回
// }
